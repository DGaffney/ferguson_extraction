class TimelineGrabber
  ...
  def perform(screen_name)
    cursor = -1
    set = client.user_timeline(screen_name, count: 200)
    status = write_block(set)
    id = set.last.id
    while !set.empty? && status
      set = client.user_timeline(screen_name, count: 200, max_id: id-1)
      status = write_block(set)
      id = set.last.id
    end
  end
  
  def write_block(block_o_tweets)
    t = block_o_tweets.map do |tweet|
      t = JSON.parse(tweet.to_json)
      t.created_at = Time.parse(t.created_at)
      t.user.created_at = Time.parse(t.user.created_at)
      t.twitter_id = t.id
      t.delete("id")
      TimelineTweet.first_or_create(twitter_id: t.twitter_id).update_attributes(t) if t.created_at > Time.parse("August 1 2014")
    end.last
    !t.nil? && !t.created_at.nil? && t.created_at > Time.parse("August 1 2014")
  end
end